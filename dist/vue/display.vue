...
<script>
  ...
  module.exports = {
    data: ()=>{
      return {
        errors:{
          ...
          textPlusInS:{ text:'span contains both text and other elements', count:0 },
          invalidAttribute:{ text: 'Invalid Attribute', count:0, instances:[] },
          invalidAttributeValue:{ text: 'Invalid Attribute Value', count:0, instances:[] },
          missingAttribute:{ text: 'Invalid Attribute', count:0, instances:[] },
        },
        ...
        defaultChangeableStyles:{
          p_font1: {
            "xml:id": "p_font1",
            "tts:fontFamily": {required:true, default:'proportionalSansSerif'},
        ...
        attributes:{
          tt:{
            optional:[],
            required:[
              "xmlns","xmlns:ttm","xmlns:tts","xmlns:ttp","xmlns:xml","xmlns:ebutts","xmlns:itts",
              "xmlns:rosetta","ttp:timeBase","ttp:cellResolution","xml:space",
              "ttp:frameRate","ttp:frameRateMultiplier","xml:lang"
            ],
            values:{
          ...
          region:{
            optional:[],
            required:['xml:id', 'tts:extent', 'tts:origin', 'tts:displayAlign', 'style']
          },
          styling:{
            optional:[],
            required:[]
          ...
          style:{
            optional:[
              'style', 'ebutts:linePadding', 'tts:lineHeight', 'tts:fontSize', 
              'tts:displayAlign', 'tts:extent', 'tts:origin',
              'tts:backgroundColor', 'tts:showBackground', 'tts:fontStyle', 'tts:fontWeight',
              'tts:color', 'tts:fontFamily', 'tts:textAlign', 'itts:fillLineGap',
              'tts:textDecoration', 'tts:textOutline', 'itts:forcedDisplay', 'ebutts:multiRowAlign', 
              'tts:ruby', 'tts:rubyReserve', 'tts:rubyAlign', 'tts:rubyPosition', 'tts:textCombine', 'tts:shear', 'tts:textEmphasis', 'tts:writingMode',
              'tts:direction', 'tts:wrapOption'],
            required:['xml:id']
          },
          body:{
            optional:[],
    ...
    methods:{
              ...
              if (typeof obj !== 'object') {
                ...
                for (key in obj) {
                  ...
                  if (key === attrkey) {
                    ...
                    if (typeof child === 'string' && options.cdata && requiresCDATA(child)) {
      ...
      timestamp(text){
        let html = '';
        html += `<p class="timing">${text} :time:${((new Date()).valueOf() - this.startTime)}ms`;
        return html;
      },
      ...
      processXml(file, xml){
        ...
        this.xml2js.parseStringPromise(xml, parseOptionsFull)
          .then((result)=>{
            ...
            this.xml2js.parseStringPromise(xml, parseOptionsSimple)
              .then((result)=>{
        ...
        for (let i = 0; i < attribs.length; i++){
          ...
          if (x < 0){
            ...
            if (x < 0){
              ...
              this.errors.invalidAttribute.instances.push(`${attribs[i]} on ${type}`)
            } else {
              opt.splice(x, 1);
            }
          } else {
          ...
          if (this.attributes[type].values && 
            ...
            err += `Value is [${attrs[attribs[i]]}] but should be [${this.attributes[type].values[attribs[i]]}]</p>`;
            this.errors.invalidAttributeValue.count++;
            this.errors.invalidAttributeValue.instances.push(`${attribs[i]} on ${type} should be ${this.attributes[type].values[attribs[i]]} but is ${attrs[attribs[i]]}`);
          }
        }
        if (req.length){
          err += `<p class="error">Missing Attribute(s) ${req.toString()} on ${type}</p>`;
          this.errors.missingAttribute.count++;
          this.errors.missingAttribute.instances.push(`${req.toString()} on ${type}`);
        }
        return err;
      },
      ...
      testStructure(nodes, path){
        ...
        for (let i = 0; i < nodes.length; i++){
          ...
          for (let i = 0; i < childnames.length; i++){
            ...
            if (this.children[newpath]) html += this.testStructure(node[childname], newpath);
          }

          if (required.length) html += `<p class="error">Required Child(ren) ${required.toString()} missing for ${path}</p>`;
        }
        return html;
      },
      ...
      testImscRosetta(file, xml, json){
        ...
        do {
...

          {
            ...
            }
            {
              ...
              if (!m['rosetta:format'] || m['rosetta:format'].length !== 1){
                html += '<p class="error">No tt.head.metadata.rosetta:format element</p>';
                break;
              }
              if (m['rosetta:format'][0].$){
                html += '<p class="error">Attributes on tt.head.metadata.rosetta:format element</p>';
                break;
              }
              if (!m['rosetta:version'] || m['rosetta:version'].length !== 1){
                html += '<p class="error">No tt.head.metadata.rosetta:version element</p>';
                break;
              }
              if (m['rosetta:version'][0].$){
                html += '<p class="error">Attributes on tt.head.metadata.rosetta:version element</p>';
                break;
              }
            }
            {
              ...
              if (styg.$){
                html += '<p class="error">Attributes found on tt.head.styling element</p>';
              }
              if (!styg.style || !styg.style.length){
                ...
                for (let i = 0; i < styg.style.length; i++){
                  ...
                  if (!s.$){
                    ...
                    this.errors.missingAttribute.count++;
                    this.errors.missingAttribute.instances.push(`No attributes on style`);
                  } else {
                    let a = s.$;
                    ...
                    if (!a['xml:id']){
                      ...
                      styles[a['xml:id']] = a;
                      if (xmlidobj[a['xml:id']]) html += `<p class="error">Duplicate xml:id ${a['xml:id']} on style element</p>`;
                      xmlidobj[a['xml:id']] = s;
                      xmlids.push(a['xml:id']);
                    }
                  }
                }
                html += `<p class="info">styles found<br />:${Object.keys(styles).toString()}</p>`;
              }
              ...
              for (let i = 0; i < stylenames.length; i++){
                if (styles[stylenames[i]].style){
                  if (!this.checkstyle(styles, styles[stylenames[i]].style)) html += `<p class="error">unknown style (${styles[stylenames[i]].style})on style ${stylenames[i]}</p>`;
                }
              }
            }

            {
              ...
              let l = h.layout[0];
              if (l._){
                html += '<p class="error">text found on tt.head.layout element</p>';
              }
              if (l.$){
                html += '<p class="error">Attributes found on tt.head.layout element</p>';
              }
              if (!l.region || !l.region.length){
                ...
                for (let i = 0; i < l.region.length; i++){
                  let r = l.region[i];
                  if (!r.$){
                    html += '<p class="error">No attributes on region element</p>';
                  } else {
                    let a = r.$;
                    ...
                    if (!a['xml:id']){
                      ...
                      regions[a['xml:id']] = a;
                      if (xmlidobj[a['xml:id']]) html += `<p class="error">Duplicate xml:id ${a['xml:id']} on region element</p>`;
                      xmlidobj[a['xml:id']] = r;

                      xmlids.push(a['xml:id']);
                      if (a.style){
                        if (!this.checkstyle(styles, a.style)) html += `<p class="error">unknown style (${a.style})on region ${a['xml:id']}</p>`;
                      }
                    }
                  }
                }
                html += `<p class="info">regions found<br />:${Object.keys(regions).toString()}</p>`;
              }
            }
          }
...

          {
            ...
            if (b._){
              html += '<p class="error">non whitespace text on tt.body element</p>';
            }
            if (b.$){
            ...
            if (!b.div || !b.div.length){
              ...
              for (let i = 0; i < b.div.length; i++){
                ...
                if (d._){
                  html += '<p class="error">non whitespace text in div element</p>';
                }
                ...
                if (id === 'unknown'){
                  ...
                  divs.push(id);
                  if (xmlidobj[a['xml:id']]) html += `<p class="error">Duplicate xml:id ${a['xml:id']} on div element</p>`;
                  xmlidobj[a['xml:id']] = d;
                  xmlids.push(id);
                }

                if (!a.begin) html += `<p class="warn">missing begin on div ${id}</p>`;
                if (!a.end) html += `<p class="warn">missing end on div ${id}</p>`;
                if (!a.region) html += `<p class="error">missing region on div ${id}</p>`;
                else {
                  if (!regions[a.region]) html += `<p class="error">unknown region (${a.region}) on div ${id}</p>`;
                  else {
                    regions[a.region]._use = (regions[a.region]._use || 0)+1;
                  }
                }
                if (!a.style) html += `<p class="warn">missing style on div ${id}</p>`;
                else {
                  if (!this.checkstyle(styles, a.style)) html += `<p class="error">unknown style (${a.style})on div ${id}</p>`;
                }
                ...
                for (let j = 0; j < d.p.length; j++){
                  ...
                  if (!p.$) {
                    ...
                    if (!pa.style) html += `<p class="error">missing style on p in div ${id}</p>`;
                    else {
                      if (!this.checkstyle(styles, pa.style)) html += `<p class="error">unknown style (${pa.style}) on p in div ${id}</p>`;
                    }
                  }
                  if (undefined !== p._) html += `<p class="error">p contains text in div ${id}</p>`;

                  if (p.br) html += `<p class="error">p contains br - br ahouls be wrapped in a span - in div ${id}</p>`;
                  ...
                  if (!p.span || !p.span.length) {
                    ...
                    for (let s1 = 0; s1 < p.span.length; s1++){
                      ...
                      if (as1){
                        ...
                        if (!as1.style) html += `<p class="error">empty style in span on div ${id}</p>`;
                        else {
                          if (!this.checkstyle(styles, as1.style)) html += `<p class="error">unknown style (${as1.style}) in span on div ${id}</p>`;
                        }
                      }

                      if (sp1.br){
                        if (sp1._) `<p class="error">span wrapping br contains text on div ${id}</p>`;
                        if (as1) `<p class="error">span wrapping br has attributes on div ${id}</p>`;
                        if (sp1.span) `<p class="error">span wrapping br contains spans? on div ${id}</p>`;
                      }

                      if (sp1.span){
                        let hasruby = false;
                        if (!as1) html += `<p class="error">no attributes on outer nested span on div ${id}</p>`;
                        as1 = as1 || {};
                        let sty = as1.style || '';
                        let stys = sty.split(' ');
                        for (let i = 0; i < stys.length; i++){
                          // all styles starting with this make a span into a ruby container.
                          if (stys[i].startsWith('s_rb_po') || stys[i].startsWith('s_rb_al')){
                            hasruby = true;
                          }
                        }
                        ...
                        for (let s2 = 0; s2 < sp1.span.length; s2++){
                          let sp2 = sp1.span[s2];
                          let as2 = sp2.$;
                          if (!as2) html += `<p class="error">no attributes on inner nested span on div ${id}</p>`;
                          as2 = as2 || {};
                          html += this.checkAttributes('span', as2);
                          if (!as2.style) html += `<p class="error">empty style in ruby span on div ${id}</p>`;
                          let sty = as2.style || '';
                          let stys = sty.split(' ');
                          let hasruby = false;
                          for (let i = 0; i < stys.length; i++){
                          ...
                          if (!hasruby) html += `<p class="error">nested ruby span should be s_rb_b or s_rb_t on div ${id}</p>`;
                          if (!this.checkstyle(styles, as2.style)) html += `<p class="error">unknown style (${as2.style}) in span on div ${id}</p>`;
        ...
        for (let i = 0; i < stylenames.length; i++) {
          ...
          if (this.defaultConstantStyles[n]){
            ...
            for (let i = 0; i < keys2.length; i++){
              if (!keys2[i].startsWith('_')) {
                if (undefined === s[keys2[i]]){
                  html += `<p class="error">missing attribute on style ${n} : ${keys2[i]}="${this.defaultConstantStyles[n][keys2[i]]}"</p>`;
                } else {
                  if (this.defaultConstantStyles[n][keys2[i]] !== s[keys2[i]]){
                    html += `<p class="error">incorrect attribute on style ${n} :  - should be `+
                    `${keys2[i]}="${this.defaultConstantStyles[n][keys2[i]]}" but is ${keys2[i]}="${s[keys2[i]]}"</p>`;
                  }
                }
                delete s[keys2[i]];
              }
            ...
            if (this.defaultChangeableStyles[n]){
              ...
              for (let i = 0; i < keys2.length; i++){
                if (!keys2[i].startsWith('_')) {
                  if (undefined === s[keys2[i]] && this.defaultChangeableStyles[n][keys2[i]].required){
                    ...
                    if (undefined === s[keys2[i]]){
                      ...
                      ` e.g. ${keys2[i]}="${this.defaultChangeableStyles[n][keys2[i]].default}"</p>`;
                    }
          ...
          if (checkextras){
            ...
            for (let i = 0; i < keys2.length; i++){
              if (!keys2[i].startsWith('_')) {
                html += `<p class="error">extra attribute on style ${n} : ${keys2[i]}="${s[keys2[i]]}"</p>`;
                delete s[keys2[i]];
              }
            }
          }
      ...
      getRegionVars(r){
        ...
        if (r['tts:displayAlign'] !== 'after' && r['tts:displayAlign'] !== 'before'){
      ...
      validateRegions(regions, styles){
        ...
        for (let i = 0; i < regionnames.length; i++) {
          ...
          if (Math.abs((r.left + r.width) - (base.left + base.width)) < 1) countsame++;
          else v = 'right';
          if (Math.abs((r.top + r.height) - (base.top + base.height)) < 1) countsame++;
          else v = 'bottom';
